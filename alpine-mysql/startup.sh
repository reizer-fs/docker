...
EOF
	...
	if [ "$MYSQL_USER_DB" != "" ]; then
        for DB in $MYSQL_USER_DB ; do
            echo "[i] Creating database: $DB"
            echo "CREATE DATABASE IF NOT EXISTS \`$DB\` CHARACTER SET utf8 COLLATE utf8_general_ci;" >> $tfile
            # set new User and Password
            if [ "$MYSQL_USER" != "" ] && [ "$MYSQL_USER_PWD" != "" ]; then
            echo "[i] Creating user: $MYSQL_USER with password $MYSQL_USER_PWD"
            echo "GRANT ALL ON \`$DB\`.* to '$MYSQL_USER'@'%' IDENTIFIED BY '$MYSQL_USER_PWD';" >> $tfile
            fi
        done
	else
